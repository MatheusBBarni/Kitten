...
describe("package-local build ownership", () => {
  it("resolves the runtime entrypoint and worker from the TUI package", () => {
    expect(PACKAGE_ROOT).toBe(TUI_ROOT)
    expect(ENTRYPOINT).toBe(join(TUI_ROOT, "src", "index.ts"))
    expect(realpathSync(TREE_SITTER_WORKER_ENTRYPOINT)).toBe(
      realpathSync(join(TUI_ROOT, "node_modules", "@opentui", "core", "parser.worker.js")),
    )
  })
...
describe("readPackageVersion", () => {
  it("defaults to the package-local manifest", async () => {
