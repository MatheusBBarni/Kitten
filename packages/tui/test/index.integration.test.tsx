  ...
  it("does not register syntax parsers when the entry module is imported", async () => {
    const child = Bun.spawn({
      cmd: [
        process.execPath,
        "-e",
        'import { mock } from "bun:test"; let calls = 0; mock.module("./packages/tui/src/ui/syntaxParsers.ts", () => ({ registerSyntaxParsers: () => { calls += 1 }, resolveSyntaxPresentation: () => ({ filetype: undefined, fallback: true }), syntaxParserManifest: { capabilities: [], plaintextFallbacks: [], parsers: [] } })); await import("./packages/tui/src/index.ts"); console.log(`calls=${calls}`)',
      ],
      cwd: process.cwd(),
      stdout: "pipe",
      stderr: "pipe",
