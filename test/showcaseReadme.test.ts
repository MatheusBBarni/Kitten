...
describe("showcase README launch contract", () => {
  test("links the canonical site and documents one verified source route", () => {
    ...
    expect(occurrences(readme, primaryInstallCmd)).toBe(1)
  })
  ...
  test("names every launch gate and the V1 measurement boundary", () => {
    ...
    expect(showcaseSection).toContain("Recording availability")
  ...
  test("includes the build, section, asset, copy, star, motion, and preview smoke checks", () => {
    ...
    expect(showcaseSection).toContain("kitten-reviewed-handoff-poster.svg")
    expect(showcaseSection).toContain("copy-command.test.ts")
    expect(showcaseSection).toContain("star-count.test.ts")
    expect(showcaseSection).toContain("proof-media-state.test.ts")
    expect(showcaseSection).toContain("proof-media.test.ts")
