...
- MUST leave the other unsurfaced variants (thoughts, plan deltas, mode/usage/session) returning `null`.
</requirements>

## Subtasks
- [ ] 2.1 Add a `translateCommand` helper mapping one ACP command to the domain shape.
- [ ] 2.2 Replace the `available_commands_update` no-op with a real case using that helper.
- [ ] 2.3 Import the ACP command type under an aliased name at the adapter boundary only.
- [ ] 2.4 Remove the `available_commands_update` row from the null-variant test table.
- [ ] 2.5 Add positive translation tests, a reducer round-trip test, and extend the completeness test to cover the new event.
...
## Tests
- Unit tests:
  - [ ] An update with two commands (one with `input.hint`, one without) yields a `commands` event with both names/descriptions and `hint` set for the first and undefined for the second.
  - [ ] `_meta` present on the update and on an individual command is absent from the translated event (assert via the completeness `collectKeys`/`FORBIDDEN_ACP_KEYS` walk).
  - [ ] An update with an empty `availableCommands` array yields a `commands` event with an empty list (not `null`).
  - [ ] The remaining unsurfaced variants (`agent_thought_chunk`, `current_mode_update`, `plan_update`, `usage_update`) still translate to `null`.
- Integration tests:
  - [ ] An `available_commands_update` fed through `translateSessionUpdate` then the reducer lands the list on `SessionState.commands`.
