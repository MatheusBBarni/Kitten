...
// Invariant: full, quiet, and narrow banners expose exactly the user-visible detail their variant permits.
...
describe("WelcomeBanner full variant", () => {
  it("renders the mascot, greeting, agent rows, cwd, and hand-off on-ramp", async () => {
    const setup = await renderBanner({ variant: "full", agents: AGENTS, cwd: CWD })
    const frame = setup.captureCharFrame()

    expect(frame).toContain("╭")
    for (const line of WELCOME_MASCOT) expect(frame).toContain(line)
    expect(frame).toContain(WELCOME_GREETING)
    expect(frame).toContain("Claude Code: connecting")
  ...
  for (const state of ["connecting", "ready", "unavailable"] satisfies WelcomeAgentState[]) {
    it(`renders the exact ${state} agent state`, async () => {
      ...
      expect(setup.captureCharFrame()).toContain(`Agent: ${state}`)

      await destroyMounted(setup.renderer)
    })
  }

  it("keeps every mascot cell in the printable ASCII range", () => {
    expect(WELCOME_MASCOT.join("\n")).toMatch(/^[\x20-\x7e\n]+$/)
  })
})

describe("WelcomeBanner compact rendering", () => {
  it("renders only the branded greeting for the quiet variant", async () => {
  ...
  it("falls back to the same one-line greeting below the mascot threshold", async () => {
    const setup = await renderBanner(
      { variant: "full", agents: AGENTS, cwd: CWD },
      WELCOME_MASCOT_MIN_WIDTH - 1,
    )
    const frame = setup.captureCharFrame()
