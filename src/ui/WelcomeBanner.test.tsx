...
// Invariant: full and compact welcome variants keep the Kitten ASCII mascot while only full exposes runtime detail.
...
describe("WelcomeBanner full variant", () => {
  it("renders the ASCII kitten mascot, generic agent state, cwd, and hand-off on-ramp", async () => {
  ...
  it("keeps every mascot cell in the printable ASCII range", () => {
    expect(WELCOME_KITTEN.join("\n")).toMatch(/^[\x20-\x7e\n]+$/)
  })
})

describe("WelcomeBanner compact rendering", () => {
  it("keeps the ASCII kitten mascot but omits runtime detail for the quiet variant", async () => {
    const setup = await renderBanner({ variant: "quiet", agents: AGENTS, cwd: CWD })
    const frame = setup.captureCharFrame()

    expect(nonBlankLines(frame)).toHaveLength(WELCOME_KITTEN.length + 1)
    expect(frame).toContain(WELCOME_GREETING)
    for (const line of WELCOME_KITTEN) expect(frame).toContain(line)
    expect(frame).not.toContain("Claude Code")
  ...
  it("falls back to a one-line greeting below the mascot threshold", async () => {
    const setup = await renderBanner(
      { variant: "full", agents: AGENTS, cwd: CWD },
      WELCOME_KITTEN_MIN_WIDTH - 1,
    )
    const frame = setup.captureCharFrame()
